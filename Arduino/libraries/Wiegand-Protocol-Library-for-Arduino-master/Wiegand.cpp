...
unsigned int WIEGAND:: _readerID=0;
...
unsigned long WIEGAND::getReaderID()
{
	return _readerID;
}
...
void WIEGAND::begin()
{
  ...
  begin(2,digitalPinToInterrupt(2),3,digitalPinToInterrupt(3),20,digitalPinToInterrupt(1),21,digitalPinToInterrupt(0));
#else
  begin(2,0,3,1,20,2,21,3);
#endif
}

void WIEGAND::begin(int pinD0, int pinIntD0, int pinD1, int pinIntD1, int pinD2, int pinIntD2, int pinD3, int pinIntD3)
{
	_lastWiegand = 0;
	_cardTempHigh = 0;
	_cardTemp = 0;
	...
	pinMode(pinD2, INPUT);					// Set D2 pin as input
	pinMode(pinD3, INPUT);					// Set D3 pin as input
	...
	attachInterrupt(pinIntD2, ReadD2, FALLING);	// Hardware interrupt - high to low pulse
	attachInterrupt(pinIntD3, ReadD3, FALLING);	// Hardware interrupt - high to low pulse
}
